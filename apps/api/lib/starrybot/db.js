...
const rolesSet = async (
  ...
  let existingRows = await knex(myConfig.DB_TABLENAME_ROLES)
    .where({
      discord_guild_id,
      give_role,
    })
  ...
  if (existingRows.length) {
    results = await knex(myConfig.DB_TABLENAME_ROLES)
      .update({
        discord_guild_id,
        decimals,
        token_address,
...
const rolesDelete = async (guildId, role) => {
  await ensureDatabaseInitialized();
  try {
    await knex(myConfig.DB_TABLENAME_ROLES)
      .where({ discord_guild_id: guildId, give_role: role })
      .del();
  } catch (e) {
...
const membersAll = async (guildId) => {
  await ensureDatabaseInitialized();
  return knex(myConfig.DB_TABLENAME_MEMBERS)
    .where({
      discord_guild_id: guildId,
    })
...
const memberDelete = async ({ authorId, guildId }) => {
  ...
  try {
    await knex(myConfig.DB_TABLENAME_MEMBERS)
      .where('discord_account_id', authorId)
      .andWhere('discord_guild_id', guildId)
      .del();
  } catch (e) {
    console.warn('Error deleting row', e);
...
const addCosmosHubAddress = async (
  guildId,
  discordAccountId,
  cosmosHubAddress,
) => {
...
module.exports = {
  ...
  rolesDeleteGuildAll,
  rolesGetForCleanUp,
  inferPreferredNativeToken,
  addCosmosHubAddress,
  nativeTokensFromGuild,
