...
async function hoistInquire(traveller) {
  ...
  const preferredPrefix = await db.inferPreferredNativeToken(
    member.discord_guild_id,
  );
  if (preferredPrefix) {
...
async function tokenRuleInfo(body, client) {
  ...
  const cosmosHubAddress = await db.getCosmosHubAddressFromDiscordId({
    discordUserId,
  });
  // Some folks who used starrybot early on didn't have their Cosmos address captured
  if (!cosmosHubAddress) {
    return {
      error: 'User must use "/starry login" to capture Cosmos Hub address',
    };
  }
  const addedRemovedRoleNames = await addRemoveRoles(
    discordUserId,
    guildId,
    cosmosHubAddress,
    client,
...
async function addRemoveRoles(
  ...
  for (let role of roles) {
    ...
    try {
      if (countStakedOnly) {
        balance = await getStakedTokenBalance({
          keplrAccount: cosmosAddress,
          tokenAddress,
          network,
          extra: { staking_contract: role.staking_contract },
        });
        console.log('getting staked token balance');
      } else {
        ...
        console.log('getting regular balance', {
          keplrAccount: cosmosAddress,
          tokenAddress,
          network,
          extra: { staking_contract: role.staking_contract },
    ...
    console.log(
      `Comparing ${balance} against ${parseInt(role.has_minimum_of)}`,
    );
...
async function hoistFinalize(blob, client) {
  ...
  let cosmosHubAddress = Bech32.encode(
    'cosmos',
    Bech32.decode(account.address).data,
  );
  await db.addCosmosHubAddress(
    member.discord_guild_id,
